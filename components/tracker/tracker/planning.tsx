...
import { db, EncounterInterface, UnitInterface } from "@/db/data";
...
export default function Planning({
  ...
  const friends: any = useLiveQuery(() => db.units.toArray());
  ...
  function rollFighters(
    combattants: { Nom: string; Nombre: number; Data: any; Groupe: string }[]
  ) {
    let result: any[] = [];
          ...
          <div className="flex gap-2">
            <div style={{ width: "-webkit-fill-available" }}>
              <Autocomplete
                ...
                options={[...Bestiaire, ...friends].map((unit) => unit.Nom)}
