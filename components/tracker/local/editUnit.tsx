...
async function editFriend(newUnit: any) {
  try {
    // Edit the new friend!
    await db.units.update(newUnit.id, newUnit);
    return {
      success: true,
    };
  } catch (error) {
...
export default function EditUnit({
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    // Edit friend
    toggleModal();
    const edit = await editFriend(data);
    ...
    if (alertContainer) {
      if (edit.success) {
        createRoot(alertContainer).render(
          <Alert
            type="success"
            boldText="Mise à jour réussite !"
            normalText={`${formState.Nom} a été modifié avec succes.`}
          />
        );
      } else {
        createRoot(alertContainer).render(
          <Alert
            type="danger"
            boldText="Erreur !"
            normalText={JSON.stringify(edit.error)}
          />
        );
      }
    }
